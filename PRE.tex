...
\section{Convergence} 
    ...
    \subsection{Weak convergence of $X_s^{N,n}$ to $X_s^N$}
            \paragraph{}
       ...
       \begin{theo}[Theorem 3.1. in \cite{Leo-Szo}]\label{leo}
        ...
        \begin{theo}
            ...
            \begin{equation}
            \E\left[f\left(X_T^{N,n}\right)-f\left(X_T^N\right)\right] \leq C_N'\delta^{\mu/4-\varepsilon}
            \end{equation}                       
            with $\delta=\frac{1}{n}$ the step size.
        \end{theo}
        ...
        \begin{proof}
            ...
            \begin{equation*}
            \E\left[f\left(X_T^{N,n}\right)-f\left(X_T^N\right)\right] \leq C_f  \E\left[\left|X_T^{N,n}-X_T^N\right|^\mu\right]
            \end{equation*}
            \begin{equation*}
    ...
    \subsection{Weak convergence of $X^{N}$ to $X$}
        \paragraph{}
        The goal of this section is to estimate the weak error $\E\left[f\left(X_T\right)-f\left(X_T^N\right)\right]$ with suitable functions $f$. In order to do so, we must go back to the definition of the virtual solution of the SDE (\ref{sde}) given in \cite{Fla-Iss-Rus-2017}. Let $(\delta,p)\in K(\beta,q)$. The authors define the virtual solution of SDE (\ref{sde}) by $X_t$ such that:
    ...
    \begin{lem}\label{morrey} Let $(\delta,p)\in K(\beta,q)$ and let $v_\lambda$ be the mild solution to (\ref{pde}) in $H_p^{1+\delta}$. $\alpha = \delta - 1/p$. Exists $c>0$ such that for both $N\in\N$ and $\rho>1$ big enough, 
        \begin{equation}
        \begin{cases}
...
\begin{lem}\label{local time}
    Let $(\delta,p)\in K(\beta,q)$, t $u$, $u^N$ be the mild solutions to (\ref{pde}), (\ref{pde2}) in $H_p^{1+\delta}$, and $Y$, $Y^N$ solutions of the SDEs (\ref{virtual}), (\ref{virtual approx}).  Then we have
    \begin{multline*}
    ...
    + 2(1+(\lambda + 1)T)\varepsilon + 4c^2T K^2e^{2\rho T}\norme{b^N-b}_{H^{-\beta}_{q}}^2\varepsilon^{-1} = g(\varepsilon).
    \end{multline*}
\end{lem}

\begin{proof}
    Let $\varepsilon>0$. Corollary \ref{cor} gives us:    
    \begin{multline*}
    ...
    \end{multline*}
    where we have used Lemma \ref{morrey}, the 1-lipschitz property of $\tilde{u}$ and the $\alpha$-Hölder property of $\nabla\tilde{u}$ (with constant $2\norme{u}_{\mathcal{C}^{1,\alpha}}$). As $\theta_s(Y-Y^N)\left|Y_s - Y^N_s\right|\leq \varepsilon$, the result follows.    
    
\end{proof}

\begin{lem}
    With assumptions and notations of Lemma \ref{local time}, with $\alpha>1/2$ we have $\forall \varepsilon\leq1$
    \begin{equation}
...
\begin{proof}
    With $2\alpha-1>0$ we have $\varepsilon\leq\varepsilon^{2\alpha-1}$. By Lemma \ref{local time},
    
    \begin{equation*}
    \E\left[L^0_T(Y-Y^N)\right]\leq g(\varepsilon).
    \end{equation*}
    ...
    the minimum of $g$ on $(0,1]$ is reached in \begin{equation*}
...
\begin{theo}
    Let $f$ be $\mu$-Hölder with constant $C_f>0$ and $\mu\in(0,1]$. If $\beta < 1/2$, $\forall \varepsilon >0$, with  $(\delta,p)\in K(\beta,q)$ such that $\alpha = \delta - 1/p = 1-2\beta - \varepsilon$, exists $\xi_f$ such that for $N\in\N$, $\rho>1$, $\lambda$ big enough it holds:
    
    \begin{equation*}
    \E\left[f\left(X_T\right)-f\left(X_T^N\right)\right] \leq \xi_f \norme{b^N-b}_{H^{-\beta}_{q}}^{\mu(2-\frac{1}{1-2\beta-\varepsilon})}
    \end{equation*}    
    where $\xi_f$ is linear in $C_f$.
\end{theo}
...
\begin{proof}
    ...
    Going back to (\ref{gronwall}) we obtain \begin{equation*}
    C(N) = (\lambda + 1)\ cTKe^{\rho T}\norme{b^N-b}_{H^{-\beta}_{q}} + \sigma\norme{b^N-b}_{H^{-\beta}_{q}}^{2-1/\alpha} \leq \zeta \norme{b^N-b}_{H^{-\beta}_{q}}^{2-1/\alpha}.
    \end{equation*}   
    ...
    Finally we obtain from (\ref{jensen}):
    
    \begin{equation*}
    \E\left[f\left(X_T\right)-f\left(X_T^N\right)\right] \leq 2^\mu C_f  \E\left[\left|Y_T-Y_T^N\right|\right]^\mu
    \end{equation*}
    \begin{equation*}
...
\section{Numerical results}
    \subsection{Rate of convergence of the Euler scheme}
